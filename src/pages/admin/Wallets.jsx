    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Quản lý ví điện tử</h1>
            <p className="text-gray-600 mt-1">Theo dõi và quản lý tất cả ví người dùng</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-600">Tổng số dư Fiat</p>
                <p className="text-2xl font-bold text-gray-900">${totalFiat.toFixed(2)}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-600">Tổng nạp tiền</p>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${totalWithdraws.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
                    ...
                    <tr key={wallet.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            <div className="text-sm font-medium text-gray-900">{wallet.userName || 'N/A'}</div>
                            <div className="text-sm text-gray-500">{wallet.userEmail}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">${wallet.fiatBalance.toFixed(2)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{wallet.carbonCreditBalance.toFixed(2)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-green-600 font-medium">${wallet.totalDeposit.toFixed(2)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-red-600 font-medium">${wallet.totalWithdraw.toFixed(2)}</div>
                      </td>
              ...
              {filteredWallets.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-900">Không có ví</h3>
                  <p className="mt-1 text-sm text-gray-500">Không tìm thấy ví nào phù hợp.</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b border-gray-200">
                <h2 className="text-2xl font-bold text-gray-900">Chi tiết ví - {selectedWallet.userName}</h2>
                <p className="text-sm text-gray-600 mt-1">{selectedWallet.userEmail}</p>
              </div>
              <div className="p-6 space-y-6">
                {/* Balance Info */}
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600">Số dư Fiat</p>
                    <p className="text-2xl font-bold text-blue-600">${selectedWallet.fiatBalance.toFixed(2)}</p>
                  </div>
                  <div className="bg-green-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600">Tín chỉ Carbon</p>
                    <p className="text-2xl font-bold text-green-600">{selectedWallet.carbonCreditBalance.toFixed(2)}</p>
                  </div>
                </div>
                    ...
                    {transactions.length > 0 ? (
                      transactions.map((tx, index) => (
                        <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                          <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${tx.type === 'deposit' ? 'bg-green-100' : 'bg-red-100'}`}>
                              {tx.type === 'deposit' ? (
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">{new Date(tx.createdAt).toLocaleString('vi-VN')}</p>
                            </div>
                          </div>
