...
const CreditDetail = () => {
  ...
  const apiErrorText = 'Không tìm thấy tín chỉ';
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(apiErrorText);
      ...
      <div className="bg-white p-6 rounded shadow">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-slate-500">{credit.region} • {credit.standard}</p>
          </div>
        ...
        <div className="mt-6 flex gap-3">
          <Link to={`/market/checkout?creditId=${credit.id}`} className="px-4 py-2 bg-emerald-600 text-white rounded">Mua ngay</Link>
          <button onClick={() => navigate('/market/auctions')} className="px-4 py-2 border rounded">Xem đấu giá</button>
        </div>
      </div>
    </main>
  );
